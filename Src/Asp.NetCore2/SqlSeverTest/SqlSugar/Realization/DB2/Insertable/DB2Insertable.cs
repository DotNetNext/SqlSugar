...
namespace SqlSugar
{
    public class DB2Insertable<T> : InsertableProvider<T> where T : class, new()
    {
        ...
        {
            if (InsertObjs.Count() > 1)
                return InsertBuilder.OracleSeqInfoList.ContainsKey(seqName) ? ((InsertBuilder.OracleSeqInfoList[seqName]- InsertObjs.Count()+1)): 0;
            return Ado.GetScalar(" select ( prevval for " + seqName + ") from sysibm.dual");
        }
        protected override void PreToSql()
        {
            ...
            if (identities.HasValue() && insertCount > 1)
            {
                ...
                foreach (var seqName in identities)
                {
                    ...
                    seqBeginValue = this.Ado.GetInt(" select (nextval for "+seqName+ ") from sysibm.dual") - insertCount;

...
namespace SqlSugar.Xugu
{
    public class UtilMethods
    {
        ...
        {
            if (currentConnectionConfig.MoreSettings == null) return Convert.ToDateTime("1900-01-01");
            else if (currentConnectionConfig.MoreSettings.DbMinDate == null) return Convert.ToDateTime("1900-01-01");
            else return currentConnectionConfig.MoreSettings.DbMinDate.Value;
        }
        internal static DateTime ConvertFromDateTimeOffset(DateTimeOffset dateTime)
        {
            if (dateTime.Offset.Equals(TimeSpan.Zero)) return dateTime.UtcDateTime;
            else if (dateTime.Offset.Equals(TimeZoneInfo.Local.GetUtcOffset(dateTime.DateTime)))
                return DateTime.SpecifyKind(dateTime.DateTime, DateTimeKind.Local);
            else return dateTime.DateTime;
        }

        internal static object To(object value, Type destinationType)=> To(value, destinationType, CultureInfo.InvariantCulture);
        ...
        public static bool IsAsyncMethod(MethodBase method)
        {
            ...
            if (name.Contains("OutputAsyncCausalityEvents")) return true;
            if (name.Contains("OutputWaitEtwEvents")) return true;
            if (name.Contains("ExecuteAsync")) return true;
        ...
        internal static T To<T>(object value)=> (T)To(value, typeof(T));
        ...
        //{
        //    if (returnObj.IsNullOrEmpty()) returnObj = new T();
        //    return returnObj;
        //}
        public static object ChangeType2(object value, Type type)
        {
            ...
            if (!type.IsInterface && type.IsGenericType)
            {
                ...
                if (innerType == typeof(TimeSpan) && value!=null)
                {
                    if (value is DateTime) return ((DateTime)value).TimeOfDay;
                    if (value is DateTimeOffset) return ((DateTimeOffset)value).LocalDateTime.TimeOfDay;
                }
        ...
        internal static T ChangeType<T>(T obj, Type type)=> (T)Convert.ChangeType(obj, type);

        internal static T ChangeType<T>(T obj)=> (T)Convert.ChangeType(obj, typeof(T));
        ...
        internal static string GetPackTable(string sql, string shortName)=>string.Format(" ({0}) {1} ", sql, shortName);

        public static Func<string, object> GetTypeConvert(object value)
        {
            if (value is int || value is uint || value is int? || value is uint?) return x => Convert.ToInt32(x);
            else if (value is short || value is ushort || value is short? || value is ushort?) return x => Convert.ToInt16(x);
            else if (value is long || value is long? || value is ulong? || value is long?) return x => Convert.ToInt64(x);
            else if (value is DateTime|| value is DateTime?) return x => Convert.ToDateTime(x);
            else if (value is bool||value is bool?) return x => Convert.ToBoolean(x);
            return null;
        }

        internal static string GetTypeName(object value)=> value?.GetType().Name;

        internal static string GetParenthesesValue(string dbTypeName)
        {
            if (Regex.IsMatch(dbTypeName, @"\(.+\)")) dbTypeName = Regex.Replace(dbTypeName, @"\(.+\)", "");
        ...
        internal static object DefaultForType(Type targetType)=> targetType.IsValueType ? Activator.CreateInstance(targetType) : null;

        internal static Int64 GetLong(byte[] bytes)=> Convert.ToInt64(string.Join("", bytes).PadRight(20, '0'));
        public static object GetPropertyValue<T>(T t, string PropertyName)=> t.GetType().GetProperty(PropertyName).GetValue(t, null);
        ...
        //        try{ decode = Encoding.GetEncoding("utf-8").GetString(bytes); } catch { decode = code; }
        //        return decode;
        //    }
        //    catch { return code; }
        //}
        ...
        public static Dictionary<string, T> EnumToDictionary<T>()
        {
            ...
            if (!typeof(T).IsEnum) return dic;
            string desc = string.Empty;
            foreach (var item in Enum.GetValues(typeof(T)))
            {
                var key = item.ToString().ToLower();
                if (!dic.ContainsKey(key)) dic.Add(key, (T)item);
            }
            return dic;
        }

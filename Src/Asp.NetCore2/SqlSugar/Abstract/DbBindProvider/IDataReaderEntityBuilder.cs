...
namespace SqlSugar
{
	...
	public partial class IDataReaderEntityBuilder<T>
	{
		...
		private delegate T Load(IDataRecord dataRecord, ISerializeService serializeService);
		...
		public T Build(IDataRecord dataRecord, ISerializeService serializeService = null)
		{
			return handler(dataRecord, serializeService);
		}

		public IDataReaderEntityBuilder<T> CreateBuilder(Type type)
		{
			DynamicMethod method = new DynamicMethod("SqlSugarEntity", type,
			new Type[] { typeof(IDataRecord), typeof(ISerializeService) }, type, true);

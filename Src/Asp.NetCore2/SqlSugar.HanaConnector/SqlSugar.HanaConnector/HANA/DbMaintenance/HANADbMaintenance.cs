...
{
    public class HANADbMaintenance : DbMaintenanceProvider
    {
        #region DML
        ...
        protected override string BackupDataBaseSql
        {
            get
            {
                return "HANAdump.exe  {0} -uroot -p > {1}  ";
            }
        }
        ...
        public override bool IsAnyTable(string tableName, bool isCache = true)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Check.ExceptionEasy($"To upgrade the HANA.Data. Error:{ex.Message}", $" 请先升级HANA.Data 。 详细错误:{ex.Message}");
                    return true;
                }
                else
                {
        ...
        public override bool AddColumnRemark(string columnName, string tableName, string description)
        {
            ...
            Check.Exception(true,"HANA no support AddColumnRemark , use " + message);
            return true;
        }
        /// <summary>
        ...
        public override bool CreateDatabase(string databaseName, string databaseDirectory = null)
        {
            ...
            if (Regex.Split(connection, oldDatabaseName).Length > 2)
            {
                ...
                {
                    connection = connection.Replace(name, "database=HANA");
                }
                else
                {
            ...
            {
                connection = connection.Replace(oldDatabaseName, "HANA");
            }
        ...
        public override bool AddRemark(EntityInfo entity)
        {
            ...
            foreach (var item in columns)
            {
                ...
                {
                    var HANACodeFirst = this.Context.CodeFirst as HANACodeFirst;
                    if (item.UnderType == UtilConstants.GuidType&&item.Length==0) 
                    {
                        item.Length = 36;
                    }
                    string sql = GetUpdateColumnSql(entity.DbTableName, HANACodeFirst.GetEntityColumnToDbColumn(entity, entity.DbTableName, item))+" "+(item.IsIdentity? "AUTO_INCREMENT" : "")+" " + " COMMENT '" + item.ColumnDescription + "'";
        ...
        {
            throw new NotSupportedException("HANA IsAnyConstraint NotSupportedException");
        }
        public override bool BackupDataBase(string databaseName, string fullFileName)
        {
            Check.ThrowNotSupportedException("HANA BackupDataBase NotSupported");
            return false;
        }

...
";
        ...
        private  void BulkCopy(DataTable dt, string copyString, NhgdbConnection conn, List<DbColumnInfo> columns)
        {
            ...
            foreach (DataColumn item in dt.Columns)
            {
                ...
                else if (key?.First() == '_')
                {
                    ...
                    result.Type = NhgdbDbType.Array | type;
                }
                else
                {
                    result.Type = null;
            ...
            using (var writer = conn.BeginBinaryImport(copyString))
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    foreach (var column in columnViews)
                    {
                        ...
                        {
                            column.Type = NhgdbDbType.Double;
                        }
                        if (column.Type == null)
                        {
                            writer.Write(value);
        ...
        public class ColumnView
        {
            ...
            public NhgdbDbType? Type { get; set; }
        }
    }
}

...
{
    public class HGUpdateBuilder : UpdateBuilder
    {
                ...
                foreach (var columns in groupList.Skip((pageIndex - 1) * pageSize).Take(pageSize).ToList())
                {
                    ...
                    updateTable.Append("\r\n (" + string.Join(",", columns.Select(it =>
                    {
                        ...
                        if (dbType == null) {
                            ...
                            var isAnyType = HGDbBind.MappingTypesConst.Where(x => x.Value.ToString().ToLower() == typeName).Any();
                            if (isAnyType)
                            {
                                dbType = HGDbBind.MappingTypesConst.Where(x => x.Value.ToString().ToLower() == typeName).FirstOrDefault().Key;
                            }
                            else {
                                dbType = "varchar";
                            }

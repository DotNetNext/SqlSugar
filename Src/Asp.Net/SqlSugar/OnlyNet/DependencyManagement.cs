            ...
            if (!IsTryJsonNet)
            {
                ...
                catch
                {
                    var message = ErrorMessage.GetThrowMessage(
                        ...
                        " Newtonsoft.Json.dll 未安装或者版本冲突，按下步骤操作即可 1、从Nuget卸载所有项目的Newtonsoft.Json和SqlSugar，用Nuget重新安装即可,如果还报错在最上层 WBE层 用NUGET安装");
            ...
            if (!IsTryMySqlData)
            {
                ...
                catch
                {
                    var message = ErrorMessage.GetThrowMessage(
                     ...
                     " MySql.Data.dll  未安装或者版本冲突，按下面步骤操作即可 1、从Nuget卸载所有项目的MySql.Data.dll和SqlSugar，用Nuget重新安装即可,如果还报错在最上层 WBE层 用NUGET安装，注意.NET 4.52版本以下用户只能用MySql.Data 6.9.12 版本 记住 6.9.12 或者6.9之前的版本 ， .NET 4.52及以上用最新的");
                    throw new Exception(message);
                }
            }
        }
            ...
            if (!IsTryPgSql)
            {
                ...
                catch
                {
                    var message = ErrorMessage.GetThrowMessage(
                     ...
                     " Npgsql.dll 未安装或者版本冲突，按下面步骤操作即可 1、从Nuget卸载所有项目的Npgsql.dll和SqlSugar，用Nuget重新安装即可,如果还报错在最上层 WBE层 用NUGET安装");
                    throw new Exception(message);
                }
            }
        }
            ...
            if (!IsTryOracle)
            {
                ...
                catch
                {
                    var message = ErrorMessage.GetThrowMessage(
                     ...
                     "需要引用ManagedDataAccess.dll 未安装或者版本冲突， 按下面步骤操作即可 1、从Nuget卸载所有项目的ManagedDataAccess.dll和SqlSugar，用Nuget重新安装即可,如果还报错在最上层 WBE层 用NUGET安装");
                    throw new Exception(message);
                }
            }
        }
        ...
        public static void TrySqlite()
        {
            if (!IsTrySqlite)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    "System.Data.SQLite.dll  未安装或者版本冲突，按下面步骤操作即可 1、从Nuget卸载所有项目的System.Data.SQLite.dll 和SqlSugar，用Nuget重新安装即可,如果还报错在最上层 WBE层 用NUGET安装");
                    throw new Exception(message);
                }
            }
        }
